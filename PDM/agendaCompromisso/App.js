...
import { SquarePen, Trash, Calendar as CalendarIcon, Clock, CalendarPlus, PlusCircle } from 'lucide-react-native';
...
export default function App() {
  ...
  const handleEdit = (commitment) => {
    ...
    const commitmentDate = new Date(commitment.dia);
  ...
  const addEventToCalendar = async (item) => {
    ...
    const startDate = new Date(item.dia);
